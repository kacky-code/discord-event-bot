...
class WRNotification(DBBaseConnection):
    ...
    def get_new_wr(self):
        ...
        for wr_id in notify_ids:
            query = "SELECT maps.name, wr.nickname, wr.login, wr.score, wr.date, wr.source, wr_not.time_diff " \
            ...
            self.cursor.execute("UPDATE worldrecords_discord_notify SET notified = 1 WHERE id = ?", wr_id);
